# Scene sync · Tizita ↔ Bóveda Cube parking note

Park the Tizita-storyboard / browse / collections to Bóveda Cube
binding. Pick up after the agentic track stabilises (post Phase 4
inbox), or sooner if storyboard work blocks on it.

## What it unlocks

- A scene shot or staged in Tizita (storyboard, browse, collection)
...
- Tags flow both ways: Tizita's scene tags inform the Bóveda scene;
...
3. `POST /scenes` accepts no `worldId` and Tizita does not call it.
4. Tizita's `storyboard_ai.py` knows about Bóveda arcs but only as
   a closed list for arc suggestion; it does not write scene data.
5. No Tizita UI for "send to Cube".
...
2. **Bóveda API.**
   ...
   - Response includes `boveda_scene_id` so Tizita can store it.
...
3. **Tizita end.**
   ...
   - On select: `POST ${BOVEDA_API_URL}/scenes` with the scene's
     name, image URL, tags, chosen `worldId`, and Tizita scene id.
   - Store the returned `boveda_scene_id` on the Tizita scene row so
...
6. **Auth and federation.**
   - Use the existing `ECOSYSTEM_API_SECRET` pattern (CLAUDE.md) so
     Tizita writes are authenticated.
   - Bóveda treats Tizita as the photo authority — image URL points
     to Tizita, not copied.

## Open decisions

- **Soft delete vs hard delete on Tizita side.** If a scene is
  deleted in Tizita, do we delete in Bóveda or just unbind?
...
- **Browse-result scenes.** Browse hits are not yet "scenes" in
  Tizita's data model; they're photos. Either promote a browse
  result to a Tizita scene first, or accept ad-hoc `POST /scenes`
